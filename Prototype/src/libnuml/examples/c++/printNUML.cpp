...
* $HeadURL: http://numl.googlecode.com/svn/trunk/libnuml/examples/c++/printNUML.cpp $
...
main (int argc, char* argv[])
{
  ...
  if (oTerms == 0)
  {
    cout << "No OntologyTerms present." << endl;
    return 1;
  }
  ...
  if (rComps == 0)
    {
      cout << "No ResultComponents present." << endl;
      return 1;
    }
  ...
  for(unsigned int i=0; i<nRComps; i++){
	  ...
	  cout << "Name of ResultComponent: " << rComps->get(i)->getId()<< endl;
	  ...
	  for(unsigned int ii=0; ii<dimDesc->size(); ii++){
		  numberOfDimDesc = 1;
		  CompositeDescription *compDesc = dimDesc->get(ii);
	  ...
	  for(unsigned int ii=0; ii<dim->size(); ii++){
		  CompositeValue *compValue = dim->get(ii);
