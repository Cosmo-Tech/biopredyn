...
/**
   ...
   * with the given <code>id</code> or <code>null</code> if no such
   * {@link Rule} exists.
   * <p>
   * @see #get(long n)
   * @see #size()
...
/**
   * Returns the first child element found that has the given <code>id</code> in the model-wide SId namespace, or <code>null</code> if no such object is found.  Note that AssignmentRules and RateRules do not actually have IDs, but the {@link libsbml} interface pretends that they do:  no assignment rule or rate rule is returned by this function.
   * <p>
