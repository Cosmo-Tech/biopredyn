...
{
  return (i != NULL) ? i->getDim() : SBML_INT_MAX;
}
...
{
  return (i != NULL) ? static_cast<int>(i->isSetDim()) : 0;
}
...
{
  return (i != NULL) ? i->setDim(dim) : LIBSBML_INVALID_OBJECT;
}
...
{
  return (i != NULL) ? i->unsetDim() : LIBSBML_INVALID_OBJECT;
}
